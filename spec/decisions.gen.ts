...
export const codeform = def("a typescript framework and cli for spec-driven development that makes typescript software", {
	principles: def({
		...
		total: given("both halves reach every form at every depth, and nothing the typechecker sees is hidden from the artifact, the scaffold or the check — a guarantee that holds only part of the way down is one you cannot rely on anywhere, because a green check does not tell you which part you got"),
	}),
...
export const conformance = def("the check that a decision only narrows the spec, and never contradicts or abandons it", {
	leaf: given("a typed leaf keeps a type the spec allows"),
	choice: given("a choice resolves to cases the spec offered: narrowing to fewer is allowed, inventing one is not"),
	coverage: given("every part the spec names is covered — except a fact, which is the spec's to assert, though a decision that does restate one restates it faithfully"),
	depth: given("the walk is total: it recurses through a collection's element, an optional's value and a choice's cases, not only through named parts, so a decision cannot escape the check by nesting"),
	blame: given("a failure names the address of the concept that failed, the way the artifact addresses it, rather than blaming the root"),
	mechanism: given("equivalently, and this is how the typechecker resolves it: the decision's projection is assignable to the spec's projection, so a decision that contradicts the spec fails to compile"),
});
...
export const former = oneOf("each former produces a kind of node from its own operands", {
	...
	ref: def("points at another concept: it takes on that concept's projection, so code here is held to the same type, but not its gaps — those stay the target's to fill and are not restated at the reference", {
		target: ref(__spec.concept),
		form: given("reference"),
	}),
	of: def("always takes something on, and only ever narrows what it took; the one former that never conjures a node from nothing", {
		takesOn: oneOf("what it takes on — exactly one of these", {
			shape: ref("another concept's parts and gaps, adding new positions or carving existing ones but never dropping a gap, so a refinement only ever conforms to its target", __spec.concept),
			type: def("a leaf's type, named by a runtime token: String, Number or Boolean. A token is a runtime value on purpose — a type named in the spec's source alone would reach the typechecker but not the artifact, the scaffold or the check, and a leaf the framework silently stops holding is worse than one it never typed"),
		}),
		form: given("shape"),
	}),
	...
	oneOf: def("exactly one of a set of cases, keyed by name and closed as written: a decision may narrow to fewer cases, which is what pick does, but never add one", {
		cases: many(of("the case's name", String), of(__spec.concept)),
		form: given("choice"),
	}),
...
export const pipeline = def("how a spec becomes verified software, across two phases", {
	...
	refine: def("phase one, still in spec space: the model narrows the spec's gaps into decisions, and conformance verifies each narrowing", {
		...
		narrow: def("how a gap closes: by restating a concept with more carved out, or — for a choice, which closes by selection rather than by carving — by naming one of its cases with pick"),
...
export const projection = def("the concrete type a spec projects into for real code. Every form has a rule, so a spec projects as far down as it is specified instead of stopping at the first form that is not a plain object of parts — one rule per form, below", {
	parts: given("named parts become an object of the projected parts"),
	leaf: given("a typed leaf becomes its token's type"),
	choice: given("a choice becomes the union of its case keys, which a pick narrows to the one key it named"),
	collection: given("a collection becomes an array of its element's projection"),
	optional: given("an optional becomes an optional key, not a required one holding `undefined`"),
	reference: given("a reference projects through the concept it points at"),
	shape: given("a shape projects through its target, with its own parts laid over that target's rather than replacing them"),
	fact: given("a fact becomes the literal value it asserts, so the code has to reproduce it"),
	gap: given("an untyped gap stays open as `unknown`, so the typechecker holds the code to exactly as much as the spec chose to type — and to no less, at any depth"),
});
...
export const spec = def("what the human author writes; bound concepts that may be narrowed", {
	concepts: many(of("the name the concept is bound under, which is part of it", String), of(__spec.concept)),
});
