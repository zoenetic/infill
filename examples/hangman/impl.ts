#!/usr/bin/env tsx
/**
 * Hangman — implemented from the infill spec `hangman-artifact.yaml`.
 *
 ...
 */
const WORD_LIST = ["hangman", "typescript", "infill", "gallows", "puzzle"];
...
function narrate(outcome: GuessOutcome, letter: string): string {
  switch (outcome) {
    case "hit": return `Good — '${letter}' is in the word.`;
    case "miss": return `Sorry — no '${letter}'.`;
    case "repeat": return `You already guessed '${letter}'.`;
    case "invalid": return `Please enter a single letter a–z.`;
    case "over": return `The game is already over.`;
  }
}
...
async function runCli(): Promise<void> {
  ...
  console.log(`Guess the ${answer.length}-letter word. ${maxWrong} wrong guesses allowed.\n`);
