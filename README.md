...
Both halves are **total** — every former has a rule, at every depth. A `ref` projects through the concept it points at; an `of` lays its own parts over its target's rather than replacing them; a `maybe` becomes an optional key; a `given` becomes the literal it asserts.

Two of them carry a dial rather than a fixed rule. A `many` is multiplicity, not a list — its reading always said the container was open, so naming a key decides it: `many(of(task))` projects to `Task[]`, `many(of(String), of(task))` to `Record<string, Task>`. And a `oneOf` keeps whatever its cases carry: a case whose name is the whole of it contributes that name, so an enum-like choice is still `"todo" | "doing" | "done"`, while a case carved further contributes its payload keyed by that name. That makes a tagged union whose discriminant is the case name itself — no reserved tag key, which is the only way it could be done here. Conformance walks the same way, so a decision that swaps out a collection's element, drops the optionality off a `maybe`, guts a choice's case from the inside, or contradicts a fact fails at the exact path it happened — addressed the way `emit` addresses it, `route.stops[].note` and `shape#circle.radius` included.
...
- **More formers** — the dial from open to decided has room at its ends, e.g. a `never()` to forbid a case or assert an absence, alongside today's `def … given`. Specifying codeform in codeform also turned up a gap in the middle: `many` is the only multiplicity former and it is a list, so a *keyed* collection — a choice's cases, a spec's concepts, both keyed by a name that is part of their content — has no faithful form today.
